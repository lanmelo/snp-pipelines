...
source $steps/results_dir.sh fastqc trimgalore star rsem
...
sbatch --export=ALL -J "$name"_rsem -D $rsem_out -o $logs_dir/%x_%j.out \
	-d afterok:$star_jid $steps/rsem.sh $star_out/Aligned.toTranscriptome.out.bam $sequencing
