...
source $steps/results_dir.sh fastqc trimgalore bwa featurecounts
...
# bwa
bwa_jid=$(sbatch --export=ALL -J "$name"_bwa -D $bwa_out -o $logs_dir/%x_%j.out \
	-d afterok:$trimgalore_jid $steps/bwa.sh $file $file2 | cut -d ' ' -f 4)

# featurecounts
sbatch --export=ALL -J "$name"_featurecounts -D $featurecounts_out -o $logs_dir/%x_%j.out 
	-d afterok:$bwa_jid $steps/featurecounts.sh $bwa_out/$name.sam
