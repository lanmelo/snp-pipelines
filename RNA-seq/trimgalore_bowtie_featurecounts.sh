...
source $steps/results_dir.sh fastqc trimgalore bowtie featurecounts
...
# bowtie
bowtie_jid=$(sbatch --export=ALL -J "$name"_bowtie -D $bowtie_out -o $logs_dir/%x_%j.out \
       	-d afterok:$trimgalore_jid $steps/bowtie.sh $file $file2 | cut -d ' ' -f 4)

# featurecounts
sbatch --export=ALL -J "$name"_featurecounts -D $featurecounts_out -o $logs_dir/%x_%j.out \
       	-d afterok:$bowtie_jid $steps/featurecounts.sh $bowtie_out/$name.sam
