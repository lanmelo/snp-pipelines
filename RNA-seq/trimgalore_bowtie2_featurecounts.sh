...
source $steps/results_dir.sh fastqc trimgalore bowtie2 featurecounts
...
# bowtie2
bowtie2_jid=$(sbatch --export=ALL -J "$name"_bowtie2 -D $bowtie2_out -o $logs_dir/%x_%j.out \
       	-d afterok:$trimgalore_jid $steps/bowtie2.sh $file $file2 | cut -d ' ' -f 4)

# featurecounts
sbatch --export=ALL -J "$name"_featurecounts -D $featurecounts_out -o $logs_dir/%x_%j.out \
       	-d afterok:$bowtie2_jid $steps/featurecounts.sh $bowtie2_out/$name.sam
